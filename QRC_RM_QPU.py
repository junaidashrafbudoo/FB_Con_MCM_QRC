...
def run_circuits_on_backend(circuits):
    ...
    sampler = Sampler(backend, options={"default_shots": int(1_0000)})
...
# Running your experiment on the backend
N=2
tf = 0
np.random.seed(10)
...
qc = create_circuit(N=2, U_res=a, trajectory=seq[2000:], a_in=1.0, a_fb=1.6)
res = run_circuits_on_backend([qc])
print(res.result()[0].data.c25.get_counts())
